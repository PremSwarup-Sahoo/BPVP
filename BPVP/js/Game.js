class Game {
  ...
  handleFuel(index) {
    // Adding fuel
    pls[index - 1].overlap(fuels, function (collector, collected) {
      player.fuel = 185;
  ...
  handlePowerCoins(index) {
    pls[index - 1].overlap(powerCoins, function (collector, collected) {
      player.score += 21;
      player.update();
      //collected is the sprite in the group collectibles that triggered
      //the event
  ...
  handlePlayerControls() {
    ...
    if (!this.blast) {
      ...
      if (keyIsDown(32)) {
        //  let  bullet = createSprite(player.positionX,player.positionY,20,50)
        this.createBullets();
      }
    }
  }
